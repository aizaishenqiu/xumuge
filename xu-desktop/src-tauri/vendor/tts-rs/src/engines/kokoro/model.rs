...
fn init_session(
    ...
    // ort 2.0.0-rc.13: CPUExecutionProvider → ep::CPU
    ...
    // Builder methods return Error<SessionBuilder>; convert to Error<()> for KokoroError.
    let mut builder = Session::builder()?
        .with_optimization_level(opt_level)
        .map_err(Into::<ort::Error>::into)?
    ...
    if write_cache {
        ...
        builder = builder
            .with_optimized_model_path(cache)
            .map_err(Into::<ort::Error>::into)?;
    }
